...
export default function TvHero({
  ...
  const [isMobile, setIsMobile] = useState<boolean>(() => {
    // This check only runs once during component initialization
    return window.matchMedia("(max-width: 480px)").matches;
  });
  useEffect(() => {
    const mediaQuery = window.matchMedia("(max-width: 480px)");

    // 2. We only need the listener here for when the screen RESIZES
        ...
        <div className="flex gap-8 w-full">
          {tvDetails.poster_path && (
            <img
              ...
              alt={tvDetails.name}
              className="w-27 md:w-48 h-45 md:h-72 object-cover rounded-lg shadow-lg"
            />
          )}
          ...
          <div className="flex-1 flex flex-col justify-end mb-4">
            ...
            <div className="flex items-center gap-4 text-sm md:text-base mb-6">
              ...
              <span className="text-gray-400 bg-gray-800 px-3 py-1 rounded">
                {tvDetails.status}
              </span>
            </div>
            <div className="flex gap-4">
              <button
                onClick={onPlayClick}
                className="bg-red-600 hover:bg-red-700 text-white px-2 sm:px-6 py-0.5 sm:py-2 cursor-pointer rounded-lg font-semibold transition-colors flex items-center gap-1 md:gap-2"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
              ...
              >
                ...
                </svg>
                {
                  isMobile
                    ...
                    : "Add to Favourites" // Desktop labels
                }
              </button>
            </div>
          </div>
